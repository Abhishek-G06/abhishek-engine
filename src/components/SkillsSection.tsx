...
import { 
  Code2, FileCode, Palette, Layers,
  Server, Database, Share2, Globe,
  GitBranch, Container, PenTool, Workflow
} from "lucide-react";
...
const SkillsSection = () => {
  ...
  return (
    <section ref={sectionRef} id="skills" className="py-20 lg:py-32 relative overflow-hidden bg-transparent">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="space-y-12">
            {skillCategories.map((category) => (
              <div key={category.titleKey} data-scroll className="text-center">
                <h3 className="text-xl font-semibold text-foreground mb-6">
                  {t(category.titleKey)}
                </h3>
                <div className="inline-flex flex-wrap justify-center gap-6 px-8 py-6 rounded-2xl bg-background/10 backdrop-blur-lg border border-primary/20 shadow-[0_8px_32px_rgba(0,0,0,0.12)] hover:bg-background/15 hover:border-primary/30 transition-all duration-300">
                  {category.skills.map((skill) => {
                    const IconComponent = skill.icon;
                    return (
                      <div key={skill.name} className="group flex flex-col items-center gap-2 px-4 py-3 rounded-xl transition-all duration-300 cursor-default">
                        <div className="p-2 rounded-lg transition-all duration-300 group-hover:scale-110" style={{ '--glow-color': skill.glowColor } as React.CSSProperties}>
                          <IconComponent className="w-8 h-8 transition-all duration-300 group-hover:animate-float group-hover:drop-shadow-[0_0_12px_var(--glow-color)]" style={{ color: skill.color }} />
                        </div>
